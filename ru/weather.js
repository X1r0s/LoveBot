...
function setupWeatherMessages(bot, chatId) {
  if (!allowedChatIds.includes(chatId.toString())) {
    console.log(
      `chatId: ${chatId} не разрешён для получения уведомлений о погоде`
    ); // Лог не разрешенного id, если срабатывает, проверьте id в разрешенных выше
    return;
  }

  const times = ["12:00", "20:00"]; // Укажи ближайшее время для теста
  console.log(
    `Планируем отправку погоды для chatId: ${chatId} в ${times.join(", ")}`
  );
  ...
  times.forEach((time) => {
    ...
    cron.schedule(cronExpression, async () => {
      ...
      if (!weather) {
        bot.sendMessage(
          chatId,
          "Не удалось получить данные о погоде. Но я всё равно тебя люблю! ❤️"
        );
        return;
      }
      ...
      bot
        .sendMessage(
          chatId,
          `Сейчас в городе ${city} ${temp}°C, ${description}. ${advice} ❤️`
        ) // Отправляем сообщение с погодой и советом
        .then(() =>
          console.log(`Сообщение успешно отправлено в chatId: ${chatId}`)
        ) // Лог успешной отправки сообщенияs
