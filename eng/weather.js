...
async function fetchWeather() {
    try {
        const response = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${weatherApiKey}&units=metric`); // Request weather data
...
function setupWeatherMessages(bot, chatId) {
    ...
    console.log(`Scheduling weather updates for chatId: ${chatId} at ${times.join(', ')}`);
    ...
    times.forEach((time) => {
        ...
        cron.schedule(cronExpression, async () => {
            ...
            if (!weather) {
                bot.sendMessage(chatId, 'Unable to retrieve weather data, but I still love you! ❤️');
                return;
            }
            ...
            if (temp < 0) {
                ...
                advice = 'The weather is nice, but you might want to bring a jacket. 🌤️';
            } else {
                advice = 'It’s warm! Dress light. ☀️';
            }

            bot.sendMessage(chatId, `Currently in ${city}: ${temp}°C, ${description}. ${advice} ❤️`) // Send weather update with advice
                .then(() => console.log(`Message successfully sent to chatId: ${chatId}`)) // Log successful message delivery
...
// Setup reminders for all allowed chat IDs
